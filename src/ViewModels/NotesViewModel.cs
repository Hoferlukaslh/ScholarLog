...
public partial class NotesViewModel : ViewModelBase
{
    ...
    private void GenererTexteExportation(string format)
    {
        if (_isExportingAll)
        {
            // LOGIQUE 1 : L'utilisateur est dans la vue de base (IsListView == true)
            // -> Récupère 'AllNotes', trie par Date.
            // -> Calcule la moyenne de chaque branche et de chaque module.
            ExportPreviewText = GenererExportGlobal(format);
        }
        else
        {
            // LOGIQUE 2 : L'utilisateur est dans la vue module (!IsListView)
            // -> Utilise 'SelectedModule' et ses branches.
            // -> Calcule la moyenne des branches et la moyenne globale du module.
